...
botStart = time.time()
cl = LINE("Es4rEJRV3MZu5j1jfKT8.HE6aZ7ktwzuq0mf6SLPCMa.IF5sZkcH3voR2CTEuOdDtBWNE3APEdUu0JOqV+o/8KM=")
cl.log("Auth Token : " + str(cl.authToken))
kl = LINE("EsqJDk289bWvJ9Vu9hM3.Qp4h2VwCAdsgafSJuPDFuW.LiX04t/y6B4gv+8Y2ClPsChAa8qVgOKX+pdIAy9GxKQ=")
    ...
    try:
        ...
        if op.type == 13:
            ...
            if op.param1 in settings['inviteprotect']:
                if op.param2 not in settings['admin'] or op.param2 not in settings['bot'] or op.param2 not in settings['gm'][to]:
            ...
            if clMID in op.param3:
                ...
                cl.updateGroup(group)
                try:
                    settings['protect'][op.param1] = op.param1
                    settings['inviteprotect'][op.param1] = op.param1
                    settings['qrprotect'][op.param1] = op.param1
                    with open('temp.json', 'w') as fp:
                        json.dump(settings, fp, sort_keys=True, indent=4)
                    cl.sendMessage(op.param1, "[成功啟動防翻服務]\n->群組保護\n->群組網址保護\n->邀請保護")
                except:
                    cl.sendMessage(op.param1, "[ERROR]\n啟動防翻服務失敗請重新邀請入群或私訊作者")
                    cl.sendContact(op.param1, "u85ee80cfb293599510d0c17ab25a5c98")
                group = cl.getGroup(to)
                GS = group.creator.mid
                try:
                    if op.param1 not in settings['gm'][to]:
                        settings['gm'][to] = {}
                        settings['gm'][GS] = GS
                        with open('temp.json', 'w') as fp:
                            json.dump(settings, fp, sort_keys=True, indent=4)
        ...
        if op.type == 19:
            ...
            if op.param1 in settings["protect"]:
                if op.param2 in settings['admin'] or op.param2 in settings['bot'] or op.param2 not in settings['gm'][to]:
                    pass
                else:
                    try:
                        kl.kickoutFromGroup(op.param1,[op.param2])
        ...
        if op.type == 26 or op.type == 25:
            ...
            if sender in settings['admin']:
                ...
                if msg.text.lower().startswith("del_gm "):
                    if 'MENTION' in msg.contentMetadata.keys()!= None:
                        names = re.findall(r'@(\w+)', text)
                        mention = ast.literal_eval(msg.contentMetadata['MENTION'])
                        mentionees = mention['MENTIONEES']
                    ...
                    settings["detectMention"][to] = to
                    with open('temp.json', 'w') as fp:
                        json.dump(settings, fp, sort_keys=True, indent=4)
                    cl.sendMessage(to, "自動回應開啟")
                elif text.lower() == 'dm off':
                    del settings["detectMention"][to]
                    with open('temp.json', 'w') as fp:
                        json.dump(settings, fp, sort_keys=True, indent=4)
            ...
            if text.lower() == 'speed':
                ...
                try:
                    ...
                    if to in settings["protect"]: ret_ += "\n群組保護開啟 [ON]"
                    else: ret_ += "\n群組保護關閉 [OFF]"
                    if to in settings["inviteprotect"]: ret_ += "\n群組邀請保護 [ON]"
                    else: ret_ += "\n群組邀請保護 [OFF]"
                    if to in settings["qrprotect"]: ret_ += "\n群組網址保護 [ON]"
                    else: ret_ += "\n群組網址保護 [OFF]"
                    if settings["contact"] == True: ret_ += "\n詳細資料 [ON]"
                    else: ret_ += "\n詳細資料 [OFF]"
                    if to in  settings["reread"]: ret_ += "\n查詢收回 [ON]"
                    else: ret_ += "\n查詢收回 [OFF]"
                    if to in  settings["detectMention"]: ret_ += "\n標註回覆 [ON]"
                    else: ret_ += "\n標註回覆 [OFF]"
                    ...
                    if 'MENTION' in msg.contentMetadata.keys()!= None:
                        ...
                        for mention in mentionees:
                            if clMID in mention["M"]:
                                if to in settings["detectMention"]:
                                    contact = cl.getContact(sender)
